...
\vspace*{\fill}
Ce document à été compilé à de Lua\kern-.025em\LaTeX{}, de la classe \textsf{memoir} et des polices \TeX{} Gyre Termes et $Neo~Euler$. La bibliographie a été réalisée en utilisant le logiciel biber et le paquet \bsc{B\kern-.025em ib}\LaTeX{}.
\chapter{Notations}
...
\end{tabular}
\end{table}
\sidepar{\vspace*{-\onelineskip}Le \textup{\texttt{\backslash0}} signale la fin d'un mot ou d'une chaîne de caractères (caractère 0 en \hyphenation{ASCII} ASCII)}
...
\begin{defb}{Image d'un élément, image directe}{imagefonct}
  ...
  Par unicité de $y$, on notera $y\eqdef f(x)$. On dit que $y$ est l'image de $x$  et que $x$ est un antécédent de $y$ (par $f$)
\end{defb}
\begin{defb}{image directe}{imd}
Soit $E$ une partie de $X$, on note $f(E)$ l'ensemble des images des éléments de $E$. C'est l'\emph{image directe} de $E$ par $f$.
\end{defb}
...
De même, soit $K$ une partie de $Y$, on note $f^\leftarrow(K)$ l'ensemble des antécédents des éléments de $K$. C'est l'\emph{image directe} de $E$ par $f$
\end{defb}\sidepar{Attention, cette notation de l'image réciproque n'est pas standard et on la note plutôt $f^{-1}$. On la +-réserve pour la réciproque.}
...
%
	...
	\end{tikzpicture}%
%
	\caption{diagramme sagittal du premier comptage.}
	\label{fig:fonctbij}
  \end{figure}
<at home : ajouter les différents ensembles, expliciter la différence>
...
\begin{proof}
	...
	\[\begin{array}{l}
		(0,0) \\
		(1,0), (0,1) \\ %ce sont des couples, pas des intervalles ouverts.
		(0,2), (1,1), (2,0) \\
		(3,0), (1,2), (2,1), (0,3) \\
		\cdots
	\end{array}.\]
