...
\end{lemmab}
\begin{proof}
Comme $n$ divise $a$, il existe un entier $k$ tel que $a=kn$, de même, comme $n$ divise $b$, il existe un entier $k'$ tel que $b=k'n$. Ainsi, \[ab=\underbrace{knk'}_{\eqdef k_2\in \NN}{}\times n.\]
\end{proof}
...
\begin{proof} 
	...
	On part du couple $(0, 0)$. Dans la ligne suivante, on range $(1,0) $ et $\left]0,1\right[$. On continue avec $(2,0), (1,1), (0,2)$ (on parcourt la diagonale du point de coordonnées $(n,0)$ pour arriver au point de coordonnées $(0,n)$). Ce procédé est illustré dans la  \cref{fig:n_croix_n}.
	\begin{figure}[htb]
		\centering
...
\end{array}\]
$\phi$ est une injection, car $\forall Q \in \ZZ[X]$, où $Q \eqdef \sum_{k=1}^d b_k X^k,$ \[{{P = Q} \iff {\forall k \in \{1,\dots, d\}, a_k = b_k}}\]. C'est une surjection, car à toute suite de coefficients $(a_0,\dots, a_d)$ on peut associer un polynôme $P = \sum_{k=1}^d a_k X^k \in \ZZ[X]$. Or on a vu que comme $\ZZ$ est dénombrable, $\ZZ^{d+1}$ l'est aussi par extension (car $\NN^{d+1}$ est dénombrable), donc $\ZZ_d[X]$ est dénombrable.
...
Nous allons appliquer les concepts vu récemment pour répondre à la question suivante : $ X = \QQ \cap [0,1]$ est-il plus grand que $ Y = \QQ \cap \left]0,1\right[$ ?
...
Ainsi on peut construire une bijection $\gamma \eqdef \phi^{-1} \circ \psi : X \to Y$. 

On en déduit que $X$ et $Y$ sont équipotents. 
\end{proof}
