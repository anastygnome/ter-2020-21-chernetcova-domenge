...
\section{\'Enumérations, comptage : notion de fonction}
...
%\diagcompfonct
Si cet exemple a l'air simple, il n'est pas simpliste pour autant :  ce procédé, qui consiste à associer à chaque lettre un nombre est omniprésent en mathématique et il porte le nom \emph{d'application}. 
...
%
	\draw[|->,thick,shorten <=1cm,shorten >=1cm] (X.north) -- node [midway,above,align=center]{jour n\textsuperscript{o}} (Y.north);
%	\node[below] at (X.south) {t};
	\end{tikzpicture}%
%
...
%
	...
	\draw[->,projection] (a4) to[out=20, in=150] (b3);
%
	\draw[|->,thick,shorten <=1cm,shorten >=1cm] (X.north) -- node [sloped, anchor=center, midway,above,align=center]{comptage 2} (Y.north);
%	\node[below] at (X.south) {t};
	\end{tikzpicture}%
%
	\caption{Un point n'est pas relié à droite : est-ce une application ?}
	\label{fig:comptageinj}
\clearpage
\end{figure}
...
%
	...
	\draw[->,projection] (a4) to[out=20, in=150] (b3);
%
	\draw[|->,thick,shorten <=1cm,shorten >=1cm] (X.north) -- node [sloped, anchor=center, midway,above,align=center]{comptage 3} (Y.north);
%	\node[below] at (X.south) {t};
	\end{tikzpicture}%
%
	\caption{Un point n'est pas relié à gauche : est-ce une application ?}
	\label{fig:fonctcomptage}
\end{figure}
\clearpage
Revenons au comptage de mots pour trancher. Dans le premier cas, on a : \sidepar{\vspace{\onelineskip} le caractère \textup{\texttt{\backslash0}} signale la fin d'un mot (caractère 0 en ASCII)} 
