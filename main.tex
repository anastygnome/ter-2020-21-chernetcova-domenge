...
\tikzexternaldisable
>>>>>>> 5c66dc7 (Ajout démo partie finie ssi maj, diverses corr)
=======
>>>>>>> 375f59e (Nettoyage & ajout licence)
\begin{document}
\begin{titlingpage}
...
}{\footnotesize Carl~Friedrich~\bsc{Gauß}~\cite{gauss_cite}}
\vspace*{\fill}
...
%\diagcompfonct
Si cet exemple a l'air simple, il n'est pas simpliste pour autant :  ce procédé, qui consiste à associer à chaque lettre un nombre est omniprésent en mathématiques et il porte le nom \emph{d\index{application}'application}.
Plus remarquable encore, ce diagramme permet même de donner une première définition quasi-formelle de ce qu'est une \index{application}application. Une chose est \emph{cruciale} : chaque lettre ne doit être comptabilisée une seule et unique fois, sinon, le résultat final est faussé !
...
%	\node[below] at (X.south) {t};
	\end{tikzpicture}%
%
	\caption{\index{application}Application parité.}
	\label{fig:parité}
\end{figure}
...
%	\node[below] at (X.south) {t};
	\end{tikzpicture}%
%
	\caption{ \index{application}Application \enquote{jours de la semaine} (convention anglo-saxonne).}
	\label{fig:jourssem}
\end{figure}
...
%	\node[below] at (X.south) {t};
	\end{tikzpicture}%
%
	\caption{Un point n'est pas relié à gauche. Est-ce une \index{application}application ?}
	\label{fig:comptinj}
  \end{figure}
  %\clearpage
...
Le cas de la~\cref{fig:comptinj} n'est pas satisfaisant. Ce n'est pas une \index{application}application.
...
%	\node[below] at (X.south) {t};
	\end{tikzpicture}%
%
	\caption{Un point n'est pas relié à droite. Est-ce une \index{application}application ?}
	\label{fig:fonctcomp}
  \end{figure}
\clearpage
...
Contrairement au cas précédent, nous avions correctement compté le nombre de lettre du mot. On a simplement une valeur surnuméraire.  On peut donc sans risque accepter ce cas particulier. Comme toutes les valeurs ne sont pas atteintes, on dira que l\index{application}'application est \emph{non-surjective}
\section{Formalisation du concept d\index{application}'application}
Maintenant que l'on dispose d'une idée claire du concept d\index{application}'application, on va essayer de le formaliser. En fait, une fonction se décrit à l'aide de 3 éléments :
\begin{enumerate}
  \item
...
Nous sommes désormais en capacité de formaliser la notion d\index{application}'application :
\begin{defb}{\index{application}Application, graphe}{app}
  ...
  L'ensemble $F$ est appelé \emph{\index{graphe}graphe de l\index{application}'application} $f$, On le notera $\mathscr{G}_{f}$.
\end{defb}
...
\begin{defb}{Image d'un élément, antécédent}{imagefonct}
  Soit $X$ et $Y$ deux ensembles et $f$ une \index{application}application de $X$ dans $Y$.
...
%
  ...
  Sur ce diagramme, on a fait figurer nos ensembles de départ, $X$ et $Y$. Cette \index{application}application s'écrit donc : \[\boxed{f\eqdef \left(X,Y,\left\lbrace(a,4);(b,2);(c,1);(d;3)\right\rbrace\right)}\]
...
\begin{theoremb}{Surjection canonique associée à une application}{appsurjcan}
  ...
  est surjective, c'est la surjection canonique associée à $f$ (de $X$ dans $f(X)$)
    \end{theoremb}
  \clearpage
  \sidepar{\vspace*{5\onelineskip}Comme tout élément de $X$ a une image, si $f(X)=Y$, $X$ a au moins \enquote{autant d'éléments} que $Y$}
  \begin{figure}[htbp]
	\centering
    \begin{tikzpicture}
	\node[bullet,fill=bleu,label=left:$a$] (a1) at (0,4) {};
...
%\sidepar{\vspace{-6\baselineskip}Quelle est la puissance d'un coton-tige ? \\ -Deux ouate s.}
...
Deux ensembles de même cardinalité ont même valeur à nos yeux, on les met donc dans le même panier, ou plutôt, dans la même classe, voir~\cite{GBMac}.}
	...
	\begin{enumerate}
		\item $E \mathrel{\approx} E$, car l\index{application}'application identité est une bijection. $\mathrel{\approx}$ est réflexive.
...
\begin{theoremb}{}{assequiv}
	Soient $E, F$ deux ensembles \textbf{finis} et $\phi~: E \to F$ une \index{application}application. Si $E \mathrel{\approx} F$ alors les 3 assertions suivantes sont équivalentes~:
...
\end{theoremb}
\begin{proof}
  L\index{application}'application $f$ définie par : \[\deffunct{f}{\NN}{\NN}{n}{n+1}\] est injective, en effet, on a : \[\forall (m,n) \in \NN, m+1=n+1 \iff m=n\] pourtant, elle n'est pas surjective  : 0 n'est jamais atteint.
...
\begin{defb}{Ensemble démombrable, au plus dénombrable}{denomb}
	 ...
	 On dit que $E$ est dénombrable ssi il existe une \index{application}application $\phi~: \NN \to E $ bijective.
...
\begin{proof}
	On construit l\index{application}'application \[ \begin{array}{cccc}
	\ & \NN& \to& 2\NN \\
	\phi~: & n & \mapsto & 2n
	\end{array}.\]

  Cette \index{application}application $\phi $ est bijective car sa réciproque est donnée par :
...
\begin{proof}
	...
	\[\begin{array}{l}
		...
		\cdots
	\end{array}.\]
	Plus formellement, l\index{application}'application qui range les couples de cette manière est
	\[\begin{array}{cccc}
		\ & \NN \times \NN & \to & \NN \\
...
Soit $\prod_{i =1}^k A_i$ le produit cartésien de $k$ ensembles dénombrables. Si $\forall i \in \{1,\dots,k\}, \phi_i : A_i \to \NN$ est une bijection, alors on peut construire l\index{application}'application \[ \begin{array}{cccc}
...
L'idée de la démonstration revient à écrire l'ensemble des entiers, dans l'ordre suivant~:  $\ZZ=\left\lbrace 0,1,(-1),2,(-2),3,(-3),\dots\right\rbrace$
	...
	Pour montrer que le cardinal de $\ZZ$ est bien $\aleph_0$, on définit donc l\index{application}'application $\phi~: \NN \to \ZZ$ telle que \[ \forall k \in \NN, \
	\begin{dcases}
	\phi(2k)& = -k \\
	\phi(2k + 1)& = k+1
	\end{dcases}
...
\begin{proof}
	...
	On s'aperçoit que chaque nombre rationnel dans le tableau a un et un seul rang $r$ (sa place dans le tableau). L\index{application}'application $\phi$ qui range les rationnels $x \in \QQ$ dans le tableau \cref{fig:denombQ} est donc une bijection.
\end{proof}
\begin{figure}[!htb]
    \centering
\begin{tikzpicture}
...
On pose  $(a_0, \dots, a_d) $ une suite de ses coefficients. On définit une \index{application}application \[\phi : \begin{array}{ccc}
\ZZ[X] & \to & \ZZ^{d+1}\\
...
\section*{\texorpdfstring{\index{application}Application : bijection entre deux sous-ensembles de $\QQ$}{}}
